...
#local path
"""spark-submit --deploy-mode client --packages com.databricks:spark-xml_2.12:0.18.0 --conf spark.sql.extensions=org.apache.iceberg.spark.extensions.IcebergSparkSessionExtensions --conf spark.sql.catalog.spark_catalog=org.apache.iceberg.spark.SparkSessionCatalog --conf spark.sql.catalog.spark_catalog.type=glue --conf spark.sql.catalog.spark_catalog.glue.id=889415020100 --conf spark.sql.catalog.spark_catalog.warehouse=s3://aep-datalake-apps-dev/emr/warehouse/nonvee-uiq-info-oh --conf spark.sql.session.timeZone=America/New_York --conf spark.driver.extraJavaOptions=-Duser.timezone=America/New_York --conf spark.executor.extraJavaOptions=-Duser.timezone=America/New_York --conf spark.sql.legacy.timeParserPolicy=LEGACY --driver-cores 4 --driver-memory 32g --conf spark.driver.maxResultSize=16g --executor-cores 1 --executor-memory 5g updated_nonvee_uiq_info_job.py --job_name uiq-nonvee-info --opco oh --data_bucket aep-datalake-work-dev --batch_start_dttm_str "2024-10-25 00:00:00" --batch_end_dttm_str "2024-10-26 00:00:00" --batch_run_dttm_str "2024-10-26 00:00:00"
"""
